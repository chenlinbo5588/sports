...
0
|| checktplrefresh('./template/default/member/register.htm', './template/default/common/seccheck.htm', 1348885253, '1', './data/template/1_1_member_register.tpl.php', './template/default', 'member/register')
;?><?php include template('common/header'); ?><script src="<?php echo $this->setting['jspath'];?>register.js?<?php echo VERHASH;?>" type="text/javascript"></script>
...
<?php if($_G['gp_action'] != 'activation') { ?><?php echo $this->setting['reglinkname'];?><?php } else { ?>你的帐号需要激活<?php } ?>
</h3>
</div>
...
<td><?php echo $_G['gp_invitecode'];?><input type="hidden" id="invitecode" name="invitecode" value="<?php echo $_G['gp_invitecode'];?>" /></td>
</tr>
</table>
...
</div><?php $invitecode = 1;?><?php } if($_G['gp_action'] != 'activation') { ?>
<div class="rfm">
<table>
<tr>
...
<td class="tipcol"><i id="tip_<?php echo $this->setting['reginput']['password'];?>" class="p_tip">请填写密码</i><kbd id="chk_<?php echo $this->setting['reginput']['password'];?>" class="p_chk"></kbd></td>
</tr>
</table>
</div>
...
<td><input type="text" id="<?php echo $this->setting['reginput']['email'];?>" name="" autocomplete="off" size="25" tabindex="1" class="px" required /><br /><em id="emailmore">&nbsp;</em></td>
...
<td class="tipcol"><i id="tip_regmessage" class="p_tip">你填写的注册原因会被当作申请注册的重要参考依据，请认真填写。</i></td>
</tr>
</table>
</div>
...
<td class="tipcol"><i id="tip_<?php echo $field['fieldid'];?>" class="p_tip"><?php if($field['description']) { echo htmlspecialchars($field['description']); } ?></i><kbd id="chk_<?php echo $field['fieldid'];?>" class="p_chk"></kbd></td>
</tr>
</table>
</div>
<?php } } ?>
...
<?php if($_G['gp_action'] != 'activation') { ?><em>&nbsp;</em><?php } ?>
<button class="pn pnc" id="registerformsubmit" type="submit" name="regsubmit" value="true" tabindex="1"><strong><?php if($_G['gp_action'] == 'activation') { ?>激活<?php } else { ?>提交<?php } ?></strong></button>
<?php if($bbrules) { ?>
...
showBBRule();
<?php } if($this->showregisterform) { ?>
addFormEvent('registerform', <?php if($_G['gp_action'] != 'activation' && !($bbrules && $bbrulesforce) && !empty($invitecode)) { ?>1<?php } else { ?>0<?php } ?>);
